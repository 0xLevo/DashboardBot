...
# --- TOP 200 COINS (Excluding Stablecoins & Low Liquidity) ---
TOP_COINS = [
    ...
    'WAXP', 'GTC', 'PROM', 'ALPHA', 'STPT', 'STMX', 'OGN', 'TRB', 'LOOM', 'STORJ'
]
...
def get_signal_data(tech_data):
    ...
    # Mapping Colors (Vibrant to Pastel)
    if score >= 3: return "STRONG BUY", "#166534" 
    elif score >= 1: return "BUY", "#4ade80" 
    elif score <= -3: return "STRONG SELL", "#991b1b" 
    elif score <= -1: return "SELL", "#f87171" 
    else: return "NEUTRAL", "#475569" 

def analyze_market():
    exchange = ccxt.mexc({'enableRateLimit': True})
    results = []
    print(f"Analyzing {len(TOP_COINS)} assets... this will take a moment.")
...
def create_html(data):
    ...
    <head>
        ...
        <style>
            ...
            .coin-box {{ border-radius: 4px; transition: all 0.2s cubic-bezier(0.4, 0, 0.2, 1); border: 1px solid rgba(255,255,255,0.03); }}
            .coin-box:hover {{ transform: scale(1.08); z-index: 50; box-shadow: 0 0 20px rgba(0,0,0,0.6); border-color: rgba(255,255,255,0.3); }}
            ...
            .modal-content {{ background: #0f172a; margin: 8% auto; padding: 32px; border-radius: 24px; width: 360px; border: 1px solid #334155; box-shadow: 0 0 40px rgba(0,0,0,0.8); }}
        ...
        <div class="p-4 md:p-8">
            <header class="flex flex-col lg:flex-row justify-between items-center pb-8 border-b border-slate-800 mb-8 gap-6">
                ...
                <div class="flex flex-wrap gap-4 items-center justify-center">
                    <input type="text" id="search-input" placeholder="Search Asset (e.g. BTC)..." onkeyup="filterCoins()" class="bg-slate-900 border border-slate-700 text-white px-5 py-2 rounded-xl text-sm outline-none w-72 focus:border-sky-500">
                    ...
                    <div class="text-[10px] text-slate-500 font-mono bg-slate-950 px-4 py-2 rounded-full border border-slate-800">
                        SYNCED: {full_update} UTC
                    </div>
                </div>
            </header>

            <div class="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-10 lg:grid-cols-12 xl:grid-cols-14 gap-1.5" id="coin-grid">
    """
    ...
    for i in data:
        ...
        <div class="coin-box p-3 text-center cursor-pointer relative" style="background-color: {i['color']};" onclick="openModal('{encoded_data}')" data-symbol="{i['symbol']}">
            <div class="flex justify-start">
                <span class="star-btn text-slate-200 opacity-30 hover:opacity-100" onclick="toggleFav(event, '{i['symbol']}')">★</span>
            </div>
            ...
            function openModal(encodedData) {
                ...
                document.getElementById('m-body').innerHTML = `
                    <div class="p-4 bg-slate-950 rounded-xl border border-slate-800">
                        <div class="text-slate-500 text-[10px] mb-1 uppercase font-bold">Market Price</div>
                        <div class="font-mono text-2xl font-bold text-white">$${data.price}</div>
                    </div>
                    <div class="p-4 bg-slate-950 rounded-xl border border-slate-800">
                        <div class="text-slate-500 text-[10px] mb-1 uppercase font-bold">Analysis Signal</div>
                        <div class="font-black text-xl" style="color:${data.color}">${data.signal_type}</div>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800">
                            <div class="text-slate-500 text-[10px] mb-1">RSI (14)</div>
                            <div class="font-bold text-lg text-white">${data.rsi}</div>
                        </div>
                        <div class="bg-slate-950 p-4 rounded-xl border border-slate-800">
                            <div class="text-slate-500 text-[10px] mb-1">MACD</div>
                            <div class="font-bold text-lg text-white">${data.macd}</div>
                        </div>
                    </div>
                `;
